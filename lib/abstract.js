...
function recordIncomingPayment(destinationTag, currency, amount, fn) {
  getHostedAddress(destinationTag, function(err, address) {
    ...
    api.rippleTransactions.create({ 
      ...
      api.externalTransactions.create({
        ...
        external_account_id: destinationTag  
      }, function(err, withdrawal) {
        if (err && fn) { fn(err, null); return; };
        if (fn) { fn(null, withdrawal); };
      });
    });   
  });
...
function getPendingWithdrawalsFull(fn) {
  var query = "select * from external_transactions et where status = 'pending'";
  query += "inner join external_accounts ea on et.external_account_id = ea.id";
...
function constructPayment(destinationAccount, amount, currency, fn) {
