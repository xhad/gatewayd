...
OutgoingPayment.prototype = {
  ...
  _recordAcceptanceOrRejectionStatus: function(payment) {
    ...
    function parseSourceBalanceChanges(payment) {
      var changes = payment.source_balance_changes;
      var assetChanges;
      if (changes.length === 1) {
        return changes[0];
      } else if (changes.length > 1) {
        assetChanges = _.reject(changes, function(change) {
          return change.currency === 'XRP'
        });
        return {
          currency: assetChanges[0].currency,
          amount: _.reduce(assetChanges, function(memo, change) {
    ...
    function parseDestinationBalanceChanges(payment) {
      var changes = payment.destination_balance_changes;
      return {
        currency: changes[0].currency,
        amount: _.reduce(changes, function(memo, change) {
          return memo + Number(change.value);
        }, 0)
      }
    }

    var sourceChange = parseSourceBalanceChanges(payment);
    _this.record.from_amount = sourceChange.amount;
    _this.record.from_currency = sourceChange.currency;

    var destinationChange = parseDestinationBalanceChanges(payment);
    _this.record.to_amount = destinationChange.amount;
